...
| `maximum-effort` | stable | Frontier-led task runner optimized for accepted quality per token. Sol or Opus owns judgment, integration, and the final result; Luna or Haiku handles bounded factual scouting, while Terra or Sonnet handles exact reversible edits with locked behavior and deterministic checks. Delegation must cost less than direct execution, risky work stays frontier-owned, and a weak-model failure triggers immediate owner takeover rather than another repair round. L uses `.maximum-effort/plan.md`; Forge may use the same leaf-allocation policy without giving up its wiki, phase, gate, or ship lifecycle. Hands PRs to `lizard` and `shepherd`. |
