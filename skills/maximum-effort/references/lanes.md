...
| Worker, design step | `Agent(subagent_type: "worker", model: "fable", prompt)` | the call's `model` beats the file's, same as the risky-step row. |
...
| Worker, design step | `spawn_agent` with `model: "gpt-5.6-sol"`, `fork_turns: "none"` | per call |
