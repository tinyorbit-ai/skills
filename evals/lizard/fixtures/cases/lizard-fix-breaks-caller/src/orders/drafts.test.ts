...
describe("saveDraft", () => {
  it("saves a draft the customer already owns", async () => {
  ...
  it("rejects an orphaned draft id", async () => {
    configureClient(stubClient([[]]));
    await expect(
      saveDraft({ draftId: "draft-9", customerId: "cust-1", items: [] }),
    ).rejects.toThrow("Unauthorized");
  });
});
...
describe("submitDraft", () => {
  ...
  it("rejects an orphaned draft id", async () => {
    configureClient(stubClient([[]]));
    await expect(
      submitDraft({ draftId: "draft-9", customerId: "cust-1", items: [] }),
    ).rejects.toThrow("Unauthorized");
  });
});
