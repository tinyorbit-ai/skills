...
Repositories never talk to a driver directly; they recieve a `DbClient` through
`configureClient` so tests can substitute a stub. Catalog and orders are kept
seperate so a change to pricing never reaches product storage.
...
in the same change so no stale import is left once the rename has occured.
